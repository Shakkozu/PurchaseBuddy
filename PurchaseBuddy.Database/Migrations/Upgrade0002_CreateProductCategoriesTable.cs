...
public class Upgrade0002_CreateProductCategoriesTable : Migration
{
	...
	public override void Up()
	{
		Create.Table(ProductCategoriesTableName)
			...
			.WithColumn("user_guid").AsFixedLengthString(36).Nullable().ForeignKey("users", "guid")
			.WithColumn("name").AsString().NotNullable()
			.WithColumn("description").AsString().Nullable();
		
		Create.Table(ProductCategoriesHierarchyTableName)
			.WithColumn("id").AsInt64().NotNullable().PrimaryKey().Identity()
			.WithColumn("category_guid").AsFixedLengthString(36).ForeignKey(ProductCategoriesTableName, "guid")
			.WithColumn("user_guid").AsFixedLengthString(36).Nullable().ForeignKey("users", "guid")
